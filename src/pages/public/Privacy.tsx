...
// Standalone, public DPDP privacy notice (/legal/privacy). Progressive
// disclosure: a "Key Points" summary Card up top, then expandable sections
// (R1). Static content — the version string is embedded, not fetched.

import { useEffect, useState } from 'react';
import { ChevronDown } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Container, Card, Button, PageHeader, Stack } from '../../components/ui';
import { NOTICE_VERSION, GRIEVANCE_EMAIL } from '../../components/shared/consent-copy';
import { KEY_POINTS, SECTIONS } from './PrivacySections';

export default function Privacy() {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  useEffect(() => { document.title = 'Privacy Notice · JobMesh'; }, []);
    ...
    <Container size="md" data-notice-version={NOTICE_VERSION} style={{ paddingTop: 'clamp(24px, 5vw, 40px)', paddingBottom: 60 }}>
      ...
      <Card variant="raised" style={{ marginTop: 24 }}>
        <Stack gap={10}>
          ...
          <div>
            <Link to="/account/privacy"><Button variant="secondary">Go to Account Privacy</Button></Link>
          </div>
        </Stack>
      </Card>
    </Container>
