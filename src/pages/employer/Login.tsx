// FILE: src/pages/employer/Login.tsx
...
// (or back to the originally-requested URL via location state).
...
export default function EmployerLogin() {
  ...
  const redirectTo = (location.state as FromState | null)?.from?.pathname || '/employer';

  // Redirect once a session exists (covers both fresh login and already-logged-in).
  useEffect(() => {
    if (employerUser) navigate(redirectTo, { replace: true });
  }, [employerUser, navigate, redirectTo]);
