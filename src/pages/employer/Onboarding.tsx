// FILE: src/pages/employer/Onboarding.tsx
// One-time company setup. Lives inside RequireEmployerAuth but outside the
// onboarded guard. Redirects on mount when there is no session (→ login) or a
// company already exists (→ dashboard); while submitting it owns the transition
// so the company-set redirect cannot ping-pong with the guard (R3). On success
// it refreshes the session, then navigates to the dashboard.
...
export default function EmployerOnboarding() {
  const { employerUser, company, refreshEmployerSession } = useEmployer();
  ...
  }, [hasOnboardedConflict, navigate]);

  if (!employerUser) return <Navigate to="/employer/login" replace />;
  if (company && !isSubmitting && !hasOnboardedConflict) {
