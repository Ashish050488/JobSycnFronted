...
// Typed client for the native posting endpoints (/api/employer/jobs). Every call
// sends the employer auth cookie, reads the { posting } / { postings } body, and
// throws EmployerJobsApiError (status + code) on any non-2xx. Employer posting
// pages call only this module — never fetch() directly.
...
const postingPath = (postingId: string) => `/api/employer/jobs/${encodeURIComponent(postingId)}`;
...
export async function listEmployerPostings(
  ...
  const body = await request<{ postings: Posting[] }>(`/api/employer/jobs${query}`);
...
export async function createEmployerPosting(input: PostingCreateInput): Promise<Posting> {
  const body = await request<{ posting: Posting }>('/api/employer/jobs', {
    method: 'POST',
