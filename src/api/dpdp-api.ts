...
// Typed client for the DPDP endpoints (/api/dpdp). Every call sends the seeker
// auth cookie (except the public notice-version), reads the typed envelope, and
// throws DpdpApiError (status + code) on any non-2xx. Consent UI calls only this
...
export async function fetchNoticeVersion(): Promise<NoticeVersionInfo> {
  return request<NoticeVersionInfo>('/api/dpdp/notice-version');
}
...
export async function listConsents(
  ...
  const body = await request<{ consents: Consent[] }>(`/api/dpdp/consents${query}`);
  return body.consents;
}

export async function grantConsent(input: GrantConsentInput): Promise<Consent> {
  const body = await request<{ consent: Consent }>('/api/dpdp/consents', {
    method: 'POST',
...
export async function withdrawConsent(consentId: string): Promise<Consent> {
  const body = await request<{ consent: Consent }>(
    `/api/dpdp/consents/${encodeURIComponent(consentId)}/withdraw`,
    { method: 'POST' },
  );
  return body.consent;
}
...
export async function listRightsRequests(): Promise<RightsRequest[]> {
  const body = await request<{ requests: RightsRequest[] }>('/api/dpdp/rights-requests');
  return body.requests;
}

export async function submitRightsRequest(input: SubmitRightsRequestInput): Promise<RightsRequest> {
  const body = await request<{ request: RightsRequest }>('/api/dpdp/rights-requests', {
    method: 'POST',
    body: JSON.stringify(input),
  });
  return body.request;
