...
export default function CompanyCard({ company, adminActions }: Props) {
  ...
  const linkUrl = company.domain || company.careersUrl || '';
  const visit = () => {
    if (adminActions) return; // Don't open link in admin mode
    let u = linkUrl.trim(); if (!u) return; if (!/^https?:\/\//i.test(u)) u = `https://${u}`; window.open(u, '_blank');
  };
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{
          ...
          {!imgErr
            ? <img src={`https://logo.clearbit.com/${host(linkUrl)}?size=128`} alt={company.companyName}
