...
import { EMPLOYER_ROUTES } from '../EmployerAppLayout';
...
export default function EmployerTopNav({ isCompact, currentUser, companyName, onLogout }: Props) {
  ...
  const isActive = (path: string) => {
    if (path === EMPLOYER_ROUTES.DASHBOARD) return location.pathname === EMPLOYER_ROUTES.DASHBOARD;
  ...
  return (
    <header style={{
      ...
      <div style={{
        ...
        <Link to={EMPLOYER_ROUTES.DASHBOARD} aria-label="Go to dashboard" style={{ textDecoration: 'none', flexShrink: 0 }}>
          <BrandLogo size="md" />
        </Link>
